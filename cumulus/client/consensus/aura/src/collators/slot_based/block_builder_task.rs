	...
	CHP: consensus_common::ValidationCodeHashProvider<Block::Hash> + Send + Sync + 'static,
{
	...
	for block_index in 0..blocks_per_core {
		...
		// Use the schedule's decision for skipping
		let is_last_block_in_core = schedule.is_effective_last_block();
		...
		// Use schedule to compute authoring duration (handles time adjustment for Medium mode)
		let authoring_duration = schedule.authoring_duration(time_left_for_block, block_time);
...
///
/// Once transaction streaming is implemented, this can likely be simplified
/// to just `Normal` mode.
...
enum BlockProductionMode {
	...
	/// We halve the authoring duration on the last block to provide
	/// import buffer without skipping any blocks.
	Medium {
...
impl BlockProductionMode {
	...
	fn from_total_blocks(total_blocks: u32) -> Self {
		match total_blocks {
			0..=1 => Self::Normal,
			2..=3 => Self::Medium { time_factor: 0.5 },
			_ => Self::Fast,
		}
	}
...
impl BlockProductionSchedule {
	...
	fn is_second_to_last(&self) -> bool {
		self.block_index + 2 == self.blocks_per_core && self.blocks_per_core > 1
	}

	/// Whether to skip producing this block entirely.
	///
	/// In Fast mode, we skip the last block in the parachain slot
	...
	/// 1. In Fast mode on the last core, we skip the actual last block
	...
	///
	/// In Medium mode, we halve the time on the last block.
	/// In other modes, we use the full available time.
	fn authoring_duration(&self, time_left: Duration, block_time: Duration) -> Duration {
		let adjusted = match &self.mode {
			BlockProductionMode::Medium { time_factor }
	...
	mod mode_tests {
		...
		fn mode_selection_from_total_blocks() {
			...
			assert!(matches!(
				...
				BlockProductionMode::Medium { time_factor: 0.5 }
			));
			assert!(matches!(
			...
			// >3 blocks = Fast
			assert!(matches!(BlockProductionMode::from_total_blocks(4), BlockProductionMode::Fast));
			assert!(matches!(
				BlockProductionMode::from_total_blocks(12),
				BlockProductionMode::Fast
			));
		}
		...
		fn mode_behavior_flags() {
			...
			let medium = BlockProductionMode::Medium { time_factor: 0.5 };
			assert!(!medium.skips_last_block());

			assert!(BlockProductionMode::Fast.skips_last_block());
		}
	}
