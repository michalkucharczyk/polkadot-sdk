...
use sp_blockchain::{HeaderBackend, HeaderMetadata, TransactionPriorityModule};
...
pub fn new_full_client<TBl, TRtApi, TExec>(
	...
	executor: TExec,
	tx_priority_module: Option<TransactionPriorityModule<TBl>>,
) -> Result<TFullClient<TBl, TRtApi, TExec>, Error>
where
	TBl: BlockT,
	TExec: CodeExecutor + RuntimeVersionOf + Clone,
{
	new_full_parts(config, telemetry, executor, tx_priority_module).map(|parts| parts.0)
}
...
where
	...
	TExec: CodeExecutor + RuntimeVersionOf + Clone,
{
	new_full_parts_record_import(config, telemetry, executor, false, tx_priority_module)
}
...
where
	...
	TCl: ProvideRuntimeApi<TBl>
		...
		+ HeaderBackend<TBl>
		+ BlockchainEvents<TBl>
		+ sp_blockchain::TransactionPriorityModuleT
		+ 'static,
